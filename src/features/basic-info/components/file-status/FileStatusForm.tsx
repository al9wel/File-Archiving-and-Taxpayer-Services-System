import { useEffect } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import * as z from "zod";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Check, Loader2 } from "lucide-react";
import { Textarea } from "@/components/ui/textarea";
import type { FileStatus } from "@/types/FileStatus";

const fileStatusSchema = z.object({
    statusName: z.string().min(2, "إسم الحالة يجب أن يكون حرفين على الأقل"),
    statusDescription: z.string().optional(),
});

type FileStatusFormValues = z.infer<typeof fileStatusSchema>;

interface FileStatusFormProps {
    initialData?: FileStatus | null;
    onSubmit: (data: FormData) => void;
    onCancel: () => void;
    isLoading?: boolean;
}

export const FileStatusForm = ({ initialData, onSubmit, onCancel, isLoading }: FileStatusFormProps) => {
    const { register, handleSubmit, setValue, formState: { errors } } = useForm<FileStatusFormValues>({
        resolver: zodResolver(fileStatusSchema),
        defaultValues: {
            statusName: initialData?.statusName || "",
            statusDescription: initialData?.statusDescription || "",
        }
    });

    useEffect(() => {
        if (initialData) {
            setValue("statusName", initialData.statusName);
            setValue("statusDescription", initialData.statusDescription || "");
        } else {
            setValue("statusName", "");
            setValue("statusDescription", "");
        }
    }, [initialData, setValue]);

    const handleFormSubmit = (values: FileStatusFormValues) => {
        const formData = new FormData();
        formData.append("statusName", values.statusName);
        if (values.statusDescription) {
            formData.append("statusDescription", values.statusDescription);
        }
        onSubmit(formData);
    };

    return (
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 pt-4" dir="rtl">
            <div className="space-y-2">
                <label className="text-sm font-medium flex items-center gap-1">
                    <span className="text-destructive">*</span>
                     إسم الحالة
                </label>
                <Input
                    placeholder="أدخل إسم حالة الملف"
                    {...register("statusName")}
                    className="text-right h-12 rounded-xl bg-muted/30 border border-muted-foreground/10 focus-visible:ring-1 focus-visible:ring-destructive"
                />
                {errors.statusName && (
                    <p className="text-sm text-destructive text-right">{errors.statusName.message}</p>
                )}
            </div>

            <div className="space-y-2">
                <label className="text-sm font-medium">الوصف</label>
                <Textarea
                    placeholder="أدخل وصفاً لحالة الملف (اختياري)"
                    {...register("statusDescription")}
                    className="text-right min-h-[100px] rounded-xl bg-muted/30 border border-muted-foreground/10 focus-visible:ring-1 focus-visible:ring-destructive resize-none"
                />
                {errors.statusDescription && (
                    <p className="text-sm text-destructive text-right">{errors.statusDescription.message}</p>
                )}
            </div>

            <div className="flex items-center gap-4 pt-6">
                <Button 
                    type="submit" 
                    disabled={isLoading}
                    className="bg-destructive hover:bg-destructive/90 text-destructive-foreground rounded-xl px-8 h-12 flex-1 flex items-center justify-center gap-2 shadow-md transition-all active:scale-95"
                >
                    {isLoading ? (
                        <Loader2 className="size-5 animate-spin" />
                    ) : (
                        <Check className="size-5" />
                    )}
                    <span>{initialData ? "تحديث البيانات" : "حفظ بيانات الحالة"}</span>
                </Button>
                <Button 
                    type="button" 
                    onClick={onCancel}
                    variant="outline" 
                    className="rounded-xl px-8 h-12 bg-muted text-muted-foreground hover:bg-muted/80 transition-colors"
                >
                    إلغاء
                </Button>
            </div>
        </form>
    );
};
