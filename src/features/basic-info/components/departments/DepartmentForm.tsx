...
const departmentSchema = z.object({
    name: z.string().min(2, "إسم القسم يجب أن يكون حرفين على الأقل"),
});
...
export const DepartmentForm = ({ initialData, onSubmit, onCancel, isLoading }: DepartmentFormProps) => {
    ...
    const handleFormSubmit = (values: DepartmentFormValues) => {
        ...
        fields.forEach(field => {
            formData.append(field, values[field as keyof DepartmentFormValues]);
        });
        onSubmit(formData);
    };
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 pt-4" dir="rtl">
            ...
            <div className="flex items-center gap-4 pt-6">
                ...
                >
                    ...
                    <span>{initialData ? "تحديث البيانات" : "حفظ بيانات القسم"}</span>
                </Button>
                <Button 
                    type="button" 
                    onClick={onCancel}
