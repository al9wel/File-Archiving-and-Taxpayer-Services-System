...
const taxPayerSchema = z.object({
    ...
    fileType: z.enum(["Individual", "Company", "CharitableCompany"]),
...
type CharitableCompanyTaxPayerFormValues = z.infer<typeof taxPayerSchema>

interface CharitableCompanyTaxPayerFormProps {
    initialData?: CharitableCompanyTaxPayer | null
...
export const CharitableCompanyTaxPayerForm = ({ initialData, onSubmit, isLoading }: CharitableCompanyTaxPayerFormProps) => {
    ...
    const { register, handleSubmit, setValue, watch, reset, formState: { errors } } = useForm<CharitableCompanyTaxPayerFormValues>({
    ...
    const handleFormSubmit = (values: CharitableCompanyTaxPayerFormValues) => {
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-8" dir="rtl">
            ...
            <Card className="p-8 rounded-3xl border shadow-sm space-y-8">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-bold">البيانات الأساسية للمكلف (شركة خيرية)</h2>
                </div>
            ...
            <Card className="p-8 rounded-3xl border shadow-sm space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* By Laws Copy */}
                    <div className="space-y-3">
                        <label className="text-sm font-bold block text-right">نسخة من النظام الأساسي (عقد التأسيس)</label>
                        <div className="relative border-2 border-dashed border-muted-foreground/20 rounded-xl p-4 flex flex-col items-center justify-center group hover:border-primary/50 transition-colors cursor-pointer text-center bg-muted/5 h-[100px]">
                            <div className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center mb-1 group-hover:scale-110 transition-transform">
