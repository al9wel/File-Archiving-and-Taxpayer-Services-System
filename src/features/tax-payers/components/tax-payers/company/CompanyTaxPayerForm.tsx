...
type CompanyTaxPayerFormValues = z.infer<typeof taxPayerSchema>

interface CompanyTaxPayerFormProps {
    initialData?: CompanyTaxPayer | null
    onSubmit: (formData: FormData) => void
    isLoading?: boolean
}
interface AdminDepartmentSelectProps {
    setValue: UseFormSetValue<CompanyTaxPayerFormValues>,
    watch: UseFormWatch<CompanyTaxPayerFormValues>,
}
...
export const CompanyTaxPayerForm = ({ initialData, onSubmit, isLoading }: CompanyTaxPayerFormProps) => {
    ...
    const { register, handleSubmit, setValue, watch, reset, formState: { errors } } = useForm<CompanyTaxPayerFormValues>({
    ...
    useEffect(() => {
        ...
        if (initialData) {
            ...
            if (initialData.companyInfo?.articlesOfIncorporation) setArticlesName(initialData.companyInfo.articlesOfIncorporation.split('/').pop() || "الملف الحالي");
    ...
    const handleFormSubmit = (values: CompanyTaxPayerFormValues) => {
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-8" dir="rtl">
            ...
            <Card className="p-8 rounded-3xl border shadow-sm space-y-8">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-bold">البيانات الأساسية للمكلف (شركة)</h2>
                </div>
