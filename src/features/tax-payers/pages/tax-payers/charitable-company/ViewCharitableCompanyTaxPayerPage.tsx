...
const ViewCharitableCompanyTaxPayerPage = () => {
    const { id } = useParams()
    const navigate = useNavigate()
    const { data: payer, isLoading, isError } = useCharitableCompanyTaxPayer(id!)
    ...
    const infoItems = [
        { label: "رقم المكلف", value: data?.taxPayerInfo.id, icon: Hash },
        { label: "الاسم التجاري", value: data?.taxPayerInfo.tradeName, icon: Shield },
        ...
        { label: "نوع الملف", value: data?.taxPayerInfo.fileType === "CharitableCompany" ? "شركة خيريه" : data?.taxPayerInfo.fileType, icon: FileText },
    ]
    ...
    const documentItems = [
        ...
        { label: "النظام الأساسي (عقد التأسيس)", value: data?.charitableCompanyInfo?.byLawsCopy },
    ]
    ...
    return (
        <div className="container mx-auto animate-in fade-in duration-500 space-y-8 pt-6 pb-12" dir="rtl">
            ...
            <div className="flex justify-end gap-3">
                ...
                {canUpdate && (
                    <Button
                        onClick={() => navigate(ROUTES.DASHBOARD.TAXPAYERS.PAYERS.CHARITABLE_COMPANY.EDIT.replace(":id", id!))}
                        ...
                        <div className="space-y-2 mb-4 text-center md:text-right">
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-2">
                                ...
                                <span className="px-4 py-1 rounded-full bg-muted text-muted-foreground text-sm font-bold">
                                    {data?.taxPayerInfo.fileType === "CharitableCompany" ? "شركة خيريه" : data?.taxPayerInfo.fileType}
                                </span>
                            </div>
                        </div>
                    </CardContent>
...
export default ViewCharitableCompanyTaxPayerPage
