...
const ViewCompanyTaxPayerPage = () => {
    ...
    const infoItems = [
        ...
        { label: "الدور", value: "مكلف شركة", icon: Briefcase },
        { label: "القسم", value: data?.userInfo.department?.name, icon: Briefcase },
        { label: "نوع الملف", value: data?.taxPayerInfo?.fileType === "Company" ? "شركة" : data?.taxPayerInfo?.fileType, icon: FileText },
    ]
    ...
    const documentItems = [
        ...
        { label: "عقد التأسيس", value: data?.companyInfo?.articlesOfIncorporation },
        { label: "رخصة المحافظ", value: data?.companyInfo?.govemorLicense },
        { label: "بطاقات الشركاء", value: data?.companyInfo?.partnersIDCards },
    ]
    ...
    return (
        <div className="container mx-auto animate-in fade-in duration-500 space-y-8 pt-6 pb-12" dir="rtl">
            ...
            <div className="flex justify-end gap-3">
                ...
                {canUpdate && (
                    <Button
                        onClick={() => navigate(ROUTES.DASHBOARD.TAXPAYERS.PAYERS.COMPANY.EDIT.replace(":id", id!))}
                    ...
                    <CardContent className="px-8 pb-8 -mt-20 relative flex flex-col md:flex-row items-center md:items-end gap-6">
                        ...
                        <div className="space-y-2 mb-4 text-center md:text-right">
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-2">
                                <span className="px-4 py-1 rounded-full bg-primary/10 text-primary text-sm font-bold">مكلف شركة</span>
                                <span className="px-4 py-1 rounded-full bg-muted text-muted-foreground text-sm font-bold">
                                    {data?.taxPayerInfo?.fileType === "Company" ? "شركة" : data?.taxPayerInfo?.fileType}
                                </span>
                            </div>
                        </div>
                    </CardContent>
...
export default ViewCompanyTaxPayerPage
